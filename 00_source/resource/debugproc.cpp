...
void CDebugProc::Draw()
{
	...
	{//デバックモードがオンの時
		...
		//テキストの描画
		m_pFont->DrawText(nullptr, &m_aStr[POINT_CENTER][0], NONE_IDX, &rect, DT_CENTER, color::Black());
		m_pFont->DrawText(nullptr, &m_aStr[POINT_LEFT][0],   NONE_IDX, &rect, DT_LEFT,   color::Black());
		m_pFont->DrawText(nullptr, &m_aStr[POINT_RIGHT][0],  NONE_IDX, &rect, DT_RIGHT,  color::Black());
	}
...
{
	va_list args;
	char aString[MAX_STR];		// 指定文字格納用
	char aSaveString[MAX_STR];	// 可変引数中身格納用
